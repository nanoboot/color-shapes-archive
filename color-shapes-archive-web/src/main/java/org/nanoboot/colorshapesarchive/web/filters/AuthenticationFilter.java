...
public class AuthenticationFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        String octConfpath = System.getProperty("color-shapes-archive.confpath");
        if (octConfpath == null || octConfpath.isEmpty()) {
            String msg = "csa configuration is broken : " + "color-shapes-archive.confpath=" + octConfpath;
            throw new RuntimeException(msg);
        }
        File octagonProperties = new File(octConfpath + "/color-shapes-archive.properties");
        try ( InputStream input = new FileInputStream(octagonProperties.getAbsolutePath())) {
            ...
            ex.printStackTrace();
            throw new RuntimeException("Loading color-shapes-archive.properties failed.");
        }
